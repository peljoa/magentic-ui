...
def check_docker_running() -> bool:
    try:
        ...
        if "No such file or directory" in str(e) or isinstance(
            e, (DockerException, ConnectionError, FileNotFoundError)
        ):
            try:
                ...
                for socket_path in docker_socket_paths:
                    if os.path.exists(socket_path):
                        ...
                        try:
                            ...
                            client.ping()  # type: ignore
                            logging.debug(
                                f"Successfully connected using socket: {socket_path}"
                            )
                            return True
                        except Exception as inner_e:
                            logging.debug(
                                f"Failed to connect with socket {socket_path}: {inner_e}"
                            )
                ...
                logging.error(
                    f"Failed to connect to Docker even after trying alternative socket paths: {retry_e}"
                )
        else:
